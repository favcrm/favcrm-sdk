...
function makeApiEvent(overrides: Partial<ApiEvent> = {}): ApiEvent {
  return {
    ...
    venue: 'Room A',
    dates: [
      { id: 'd1', startTime: '2099-01-01T10:00:00Z', endTime: '2099-01-01T12:00:00Z' },
    ],
    ...overrides,
  };
}
...
describe('mapApiEvent', () => {
  ...
  it('falls back to content when no introduction', () => {
    const result = mapApiEvent(makeApiEvent({ introduction: null, content: 'Content text' }));
    expect(result.description).toBe('Content text');
  });

  it('returns empty description when neither introduction nor content', () => {
    const result = mapApiEvent(makeApiEvent({ introduction: null, content: null }));
  ...
  it('maps dates array', () => {
    const result = mapApiEvent(makeApiEvent({
      dates: [
        { id: 'd1', startTime: '2099-01-01T10:00:00Z', endTime: '2099-01-01T12:00:00Z', allDay: true, remainingQuota: 5 },
  ...
  it('parses quota string to capacity', () => {
    const result = mapApiEvent(makeApiEvent({ quota: '50' }));
    expect(result.capacity).toBe(50);
  });

  it('returns null capacity for missing quota', () => {
    const result = mapApiEvent(makeApiEvent({ quota: undefined }));
    expect(result.capacity).toBeNull();
  });

  it('returns null capacity for non-numeric quota', () => {
    const result = mapApiEvent(makeApiEvent({ quota: 'unlimited' }));
    expect(result.capacity).toBeNull();
  });
  ...
  it('detects past events from dates', () => {
    const result = mapApiEvent(makeApiEvent({
      status: 'active',
      dates: [{ startTime: '2020-01-01T10:00:00Z', endTime: '2020-01-01T12:00:00Z' }],
    }));
  ...
  it('maps image URL', () => {
    const result = mapApiEvent(makeApiEvent({ image: 'https://img.test/photo.jpg' }));
